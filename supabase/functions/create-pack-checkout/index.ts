...
const PACK_PRICE_MAP: Record<string, string> = {
  "career": "price_1Sx9o1C3xPeU0PAg4tyGgK7n",
  "unhinged": "price_1Sx9qmC3xPeU0PAg3TQs8EDG",
  "main_character": "price_1Sx9qmC3xPeU0PAgKdTJQ6PR",
};

const logStep = (step: string, details?: any) => {
  const detailsStr = details ? ` - ${JSON.stringify(details)}` : "";
  console.log(`[CREATE-PACK-CHECKOUT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${origin}/?pack_purchase=success`,

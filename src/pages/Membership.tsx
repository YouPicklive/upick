...
import { Star, Zap, MapPin, Sparkles, Bookmark, Crown, Loader2, CreditCard } from 'lucide-react';
...
export default function Membership() {
  ...
  const handleUpgrade = async () => { await upgradeToPremium(); };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-md mx-auto px-6 py-8">
        ...
        <div className={`rounded-2xl p-6 mb-6 border ${
          isPremium ? 'gradient-warm text-primary-foreground border-primary/20' : 'bg-card border-border'
        }`}>
          <div className="flex items-center gap-3 mb-3">
            {isPremium ? <Crown className="w-6 h-6" /> : <Star className="w-6 h-6 text-muted-foreground" />}
            <div>
              <h2 className="font-display text-xl font-bold">{isPremium ? 'YouPick Plus' : 'Free Plan'}</h2>
        ...
        {!isPremium && (
          <div className="bg-primary/5 border border-primary/10 rounded-2xl p-5 mb-6">
            ...
            <ul className="space-y-3 text-sm text-muted-foreground mb-5">
              ...
              <li className="flex items-center gap-3"><MapPin className="w-4 h-4 shrink-0" style={{ color: 'hsl(152, 55%, 42%)' }} /> Extended search radius</li>
